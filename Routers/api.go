...
import (
	"gin_demo/app/api/Controller"
	"gin_demo/app/api/middleware/cors"
	"gin_demo/app/global/variable"
	// validatorFactory "gin_demo/app/http/validator/core/factory"
	"gin_demo/Library/gin_release"
...
func InitApiRouter() *gin.Engine {
	...
	router.Static("/public", "./public")             //  定义静态资源路由于实际目录映射关系
	...
	vApi := router.Group("/api/")
	{
		...
		home := vApi.Group("user/")
		{
			...
			home.POST("/login", login.Login)
			home.GET("/index", login.Index)
			home.POST("/register", login.Register)
			home.POST("/user/info", user.GetUserInfo)
		}
	}
	return router
}

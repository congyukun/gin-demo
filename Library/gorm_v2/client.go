...
func getDsn(sqlType, readWrite string, dbConf ...ConfigParams) string {
	...
	ParseTime := variable.ConfigGormv2Yml.GetString("Gormv2." + sqlType + "." + readWrite + ".ParseTime")
	...
	if len(dbConf) > 0 {
		if strings.ToLower(readWrite) == "write" {
			...
			if len(dbConf[0].Write.ParseTime) > 0 {
				ParseTime = dbConf[0].Write.ParseTime
			}
		} else {
			...
			if len(dbConf[0].Read.ParseTime) > 0 {
				ParseTime = dbConf[0].Read.ParseTime
			}
		}
	}
	...
	switch strings.ToLower(sqlType) {
	case "mysql":
		return fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=%s&parseTime=%v&loc=Local", User, Pass, Host, Port, DataBase, Charset, ParseTime)
	case "sqlserver", "mssql":
